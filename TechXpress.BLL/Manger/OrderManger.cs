...
namespace TechXpress.BLL.Manger
{
    public class OrderManger : IOrderManger
    {
        ...
        public OrderManger(IOrderrepo _orderrepo, ICouponRepo _couponRepo)
        {
        ...
        public void Insert(OrderAddDto orderAddDto)
        {
            var model3 = new Order()
            {
                ...
                UserID = orderAddDto.UserID,
                 TotalAmountToPay = orderAddDto.TotalAmountToPay,
                OrderDate = orderAddDto.OrderDate,
                Order_Status = orderAddDto.Order_Status,
        ...
        public void ApplyDiscountToOrder(string Coupon, OrderUpdateDto orderUpdateDto)
        {
           
            var ValidCoupon = couponRepo.GetByCode(Coupon);
            var order = orderrepo.GetById(orderUpdateDto.OrderID);

            if (ValidCoupon.ExpirationDate.Date >= DateTime.Now.Date
                && (ValidCoupon.UsageCount ?? 0) < ValidCoupon.UsageLimit)
            {
                orderUpdateDto.TotalAmountToPay = orderUpdateDto.TotalAmountToPay - (orderUpdateDto.TotalAmountToPay *( ValidCoupon.couponDiscountinPercentage / 100));
                order.TotalAmountToPay = orderUpdateDto.TotalAmountToPay;
                orderrepo.Update(order);
                orderrepo.SaveChanges();
                ValidCoupon.UsageCount++;

                var coupon = new Coupon
                {
                    Code = ValidCoupon.Code,
                    ExpirationDate = ValidCoupon.ExpirationDate,
                    UsageLimit = ValidCoupon.UsageLimit,
                    UsageCount = ValidCoupon.UsageCount,
                    couponDiscountinPercentage = ValidCoupon.couponDiscountinPercentage
                };
